...
if (isset($_POST['update'])) {
    ...
    if (
        !empty($data['o_pass'] && !password_verify($data['o_pass'], $password))
    ...
    if (!empty($data['o_pass'] || !empty($data['n_pass']))) {
        if (
            empty($data['name_error']) && empty($data['email_error']) && empty($data['o_pass_error']) &&
            empty($data['n_pass_error']) && empty($data['phone_error']) && empty($data['address_error'])
        ) {
            $pass = password_hash($data['n_pass'], PASSWORD_DEFAULT);
            if ($source->Query(
                "UPDATE userregistration SET name=?,email=?,password=?,phone=?,address=? where id=?",
        ...
        if (empty($data['name_error']) && empty($data['email_error']) && empty($data['phone_error']) && empty($data['address_error'])) {
            if ($source->Query(
                "UPDATE userregistration SET name=?,email=?,phone=?,address=? where id=?",
